...
function getSourceRoot(relativeDir) {
...
gulp.task("build", ["build-ts", "build-sass", "build-ts-mocks"]);

gulp.task("default", ["build"]);

gulp.task("build-ts-mocks", function(){
    return gulp.src("./mocks/**/*.ts")
            .pipe(typescript({
                target: "ES5",
                module: "amd",
                sourcemap: true,
                sourceRoot: getSourceRoot("mocks/"),
                 out: "./MockShell.js",
                outDir: "./libs/mlstudio/"
            }))
            .pipe(gulp.dest("./libs/mlstudio/"));
});
