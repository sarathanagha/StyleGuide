define(["require", "exports", "knockout", "jquery"], function (require, exports, ko, $) {
    ...
    var DataSourceType = Microsoft.DataStudio.DataCatalog.Models.DataSourceType;
    var userProfileService = Microsoft.DataStudio.DataCatalog.Services.UserProfileService;
    var BaseBrowseResultsViewModel = (function () {
        function BaseBrowseResultsViewModel() {
            var _this = this;
            this.DataSourceType = DataSourceType;
            this.resx = resx;
            this.searchResult = browseManager.searchResult;
            this.isSearching = browseManager.isSearching;
            this.numResults = ko.pureComputed(function () {
                return util.stringFormat(resx.totalResultsFormat, _this.searchResult().totalResults) +
            ...
            this.exploreContainer = function (asset) {
                browseManager.exploreContainer(asset);
            };
            this.allSelected = ko.pureComputed(function () {
                var pageItems = _this.searchResult().results.filter(function (r) { return !_this.isDeleted(r); });
                if (browseManager.container() && !_this.isDeleted(browseManager.container())) {
        ...
        BaseBrowseResultsViewModel.prototype.isDeleted = function (dataEntity) {
            return browseManager.isAssetDeleted(dataEntity);
        };
        BaseBrowseResultsViewModel.prototype.isSelected = function (dataEntity) {
            return browseManager.multiSelected().some(function (s) { return s.__id === dataEntity.__id; });
        };
        BaseBrowseResultsViewModel.prototype.multiSelectAsset = function (dataEntity, event) {
            event.ctrlKey = true;
            this.selectAsset(dataEntity, event);
        };
        BaseBrowseResultsViewModel.prototype.selectAsset = function (dataEntity, event) {
            if (this.isDeleted(dataEntity)) {
                return;
            }
            if (event.ctrlKey) {
                // Multi select mode
                if (browseManager.multiSelected().some(function (a) { return a.__id === dataEntity.__id; })) {
                    // Selected an already selected multi selection => remove from select list
                    var filtered = browseManager.multiSelected().filter(function (s) { return s.__id !== dataEntity.__id; });
        ...
        BaseBrowseResultsViewModel.prototype.saveRecentlySelected = function () {
            if (browseManager.multiSelected().length) {
                var selectedItems = [];
                var created = new Date().toISOString();
                browseManager.multiSelected().forEach(function (i) {
                    var item = {
                        assetId: i.__id,
                        createdDate: created,
                        id: util.createID(),
        ...
        BaseBrowseResultsViewModel.prototype._getAssetKey = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.formatWatermark = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.formatObjectType = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.getSourceTypeImage = function (dataEntity) {
            switch (util.plainText(dataEntity.dataSource.sourceType).toLowerCase()) {
                case "sap hana":
                case "sql server analysis services":
                case "sql server analysis services multidimensional":
        ...
        BaseBrowseResultsViewModel.prototype.formatContainedInUrl = function (dataEntity) {
            if (!dataEntity.dsl.address.url) {
                return "";
            }
        ...
        BaseBrowseResultsViewModel.prototype.getContainedInName = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.formatSourceAndObjectTypeText = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.getExploreContainerText = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.formatSourceTypeText = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.formatObjectTypeText = function (dataEntity) {
        ...
        BaseBrowseResultsViewModel.prototype.detectAcronyms = function (label) {
            var knownAcronyms = ["kpi", "sql"];
            var re = new RegExp("\\b(" + knownAcronyms.join("|") + ")\\b", "ig");
            return (label || "").replace(re, function (match) { return match.toLocaleUpperCase(); });
        };
        BaseBrowseResultsViewModel.prototype.onPagingChanged = function (newPage) {
        ...
        BaseBrowseResultsViewModel.prototype.clearSearch = function () {
        ...
        BaseBrowseResultsViewModel.prototype.toggleSelectAll = function (d, event) {
            var _this = this;
            ...
            if (this.allSelected()) {
                ...
                var pageItems = this.searchResult().results.filter(function (r) { return !_this.isDeleted(r); });
        ...
        BaseBrowseResultsViewModel.prototype.isHighlighted = function (value) {
            return (value || "").indexOf("tokyo-highlight") > 0;
        };
        return BaseBrowseResultsViewModel;
    })();
    exports.BaseBrowseResultsViewModel = BaseBrowseResultsViewModel;
});
//# sourceMappingURL=BaseBrowseResultsViewModel.js.map