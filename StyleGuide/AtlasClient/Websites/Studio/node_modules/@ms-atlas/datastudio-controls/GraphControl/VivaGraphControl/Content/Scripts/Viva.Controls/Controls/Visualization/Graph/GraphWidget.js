        ...
        */
        ...
        var Widget = (function (_super) {
            ...
            function Widget(element, options, createOptions) {
                ...
                this._entityFocusIn = function (e) {
                    var element = document.activeElement;
                    var htmlElement = element;
                    var svgElement = element;
                    // Don't adjust if a container element gains focus.
                    if ($(htmlElement).hasClass(graphContainerClassName) || svgElement.className.baseVal === graphEdgeSvgClassName) {
                        return;
                    }
                    // grab the bounds of the active element in relation to the graph overlay.
                    // Values are automatically adjusted for scale. bringRectIntoView() should handle whether or not a value is scaled so we adjust the value.
                    var elementX = ($(htmlElement).offset().left - $(graphOverlaySelector).offset().left) / _this.scale();
                    var elementY = ($(htmlElement).offset().top - $(graphOverlaySelector).offset().top) / _this.scale();
                    var elementWidth = ((htmlElement.clientWidth > 0) ? htmlElement.clientWidth : 1);
                    var elementHeight = ((htmlElement.clientHeight > 0) ? htmlElement.clientHeight : 1);
                    _this.bringRectIntoView({ x: elementX, y: elementY, width: elementWidth, height: elementHeight }, false);
                };
                /**
                ...
                this._canvasMouseDown = function (canvasViewModel, e) {
                    ...
                    _this.interactionStateMachine.handleAction(GraphWidgetConstants.InteractionAction.MouseDown, e);
                    // We do not want to prevent default so the graph control still correctly gets focus without
                    // us having to explicitly call focus.
                    return true;
                };
            ...
            */
             ...
             */
            Widget.prototype.bringRectIntoView = function (rect, animate) {
                if (animate === void 0) { animate = true; }
                ...
                if (rect) {
                    ...
                    if (shouldAnimate) {
                        if (animate) {
                            ...
                            this._setOriginAndZoom({ x: targetX * targetScale, y: targetY * targetScale }, targetScale);
                        }
                    }
                }
                return Q(true);
