(function (uvo) {
    ...
    //uvo.addEarlyInit(uvo.simpleRouteFactory("khCarLocation", "/ccw/kh/carLocation.do", /kh\/findVehicle/i));
    ...
    var module = {
        init : function () {
            docReady.done(function () {
            	tempUnits = 'F'; //or 'C'
            	initSchedule = {"serviceResponse":[{"climateIndex":0,"reservHvacInfoDetail":{"airTemp":{"unit":1,"value":{"hexValue":"01"}},"climateSet":"false","defrost":"false","reservInfo":{"day":["9"],"time":{"time":"0000","timeSection":0}}},"schedId": 0},{"climateIndex":1,"reservHvacInfoDetail":{"airTemp":{"unit":1,"value":{"hexValue":"01"}},"climateSet":"false","defrost":"false","reservInfo":{"day":["9"],"time":{"time":"0000","timeSection":0}}},"schedId": 0}]};
        ...
        initPendingRefresh: function() {
        	...
        	setTimeout(function(){ return setInterval(function(){window.location = window.location.pathname + "?r" ;},delay); },delay);
        },
        ...
        initRemoteClimate    : function initRemoteClimate() {
			...
			function loadRequest() {
				uvo.dataRequest("khVehicleStatusRemote").done(function (statusResponse) {
	                ...
	                if (isPending) {
						...
						if (params == 'r') { uvo.clearLoadingConfirm(); }
			...
			function loadState(state) {
				...
				if (state == 'error') {
					module.clearPendingRefresh(interval);
					$('.kh-button-error').removeClass('hide');
            ...
            function updateClimateButton() {
                ...
                if (!isPending) updatePendingList('climate');
            }
            ...
            function updatePendingList(action) {
            	...
            	if (isClimateModified || isDefrostModified || isEngineModified || (isDialModified && engineOn)) {                		
            		if (!isPending) $('.kh-button-submit').removeClass('disabled'); 
...
//                }
        ...
        climateDialModule : {
        	hexTempMaps: {
        		...
        		c: {
        			ticks : 16,
        			tempMapC : ["20","1E","1C","1A","18","16","14","12","10","0E","0C","0A","08","06","04","03","01"],
        	...
        	init:function(selector) {
    			...
    			function enable(toggle) {
    				if (!toggle) {
    					...
    					knob.remove();
    					knob = paper.image('/images/img/remote/odo-button.png',0,0,68,68);
    					moveKnob(knobX,knobY,knobA,rPathString);
    				}
    			}
            	...
            	if(data.success === true) 								//Vehicle has schedule
            	{
            		if(data.serviceResponse.length == 2) 				//Vehicle has both schedules
            			...
            			if(data.serviceResponse[0].climateIndex == 0)	//Vehicle has Schedule I, not Schedule II
            				data.serviceResponse.push({"climateIndex":1,"reservHvacInfoDetail":{"airTemp":{"unit":1,"value":{"hexValue":"01"}},"climateSet":"false","defrost":"false","reservInfo":{"day":["9"],"time":{"time":"0000","timeSection":0}}},"schedId": 0});
            			if(data.serviceResponse[0].climateIndex == 1)	//Vehicle has Schedule II, not Schedule I
            				data.serviceResponse.unshift({"climateIndex":0,"reservHvacInfoDetail":{"airTemp":{"unit":1,"value":{"hexValue":"01"}},"climateSet":"false","defrost":"false","reservInfo":{"day":["9"],"time":{"time":"0000","timeSection":0}}},"schedId": 0});
	        ...
	        function initComponents($schedule, data, dayHvac) {
	        	...
	        	if (tempValue) {
		        	if(tempValue.toString().toLowerCase() == "high")
		    	    	tempValue = 90;
		    	    if(tempValue.toString().toLowerCase() == "low")
		    	    	tempValue = 62;
	        	}
...
//				        $('.ui-sliderC-range', $schedule).width((newTempC-17)*30);
        ...
        addHvacEvent         : function($schedule, schedId, dayHvac, numberHvac, dates, updateValuesHelper) {
			...
			var hvacInfos = {"climateIndex": ($schedule.hasClass('schedule-one'))?0:1, "reservHvacInfoDetail" : {"airTemp" : {"unit" : 1, "value" : {"hexValue": "01"}}, "climateSet" : true, "defrost" : true, "reservInfo" : {"day" : [], "time" : {"time" : "", "timeSection" : 0}}},"schedId":0};
            ...
            if (dateSelected){
                ...
                if(module.validScheduleRule(numberHvac, newNumberHvac, dayHvac, hvacInfos.reservHvacInfoDetail.reservInfo.day)){ 
                	...
                	module.postHvac(mySchedule, $schedule);
                	 ...
                	 if(timePeriod == 0) {
                		 $(".time-hvac", $schedule).html(newTime.fontcolor('#444444') + ' a.m.');
                	 }
                	 else {
                		 $(".time-hvac", $schedule).html(newTime.fontcolor('#444444') + ' p.m.');
                	 }
        ...
        initRemoteLock       : function initRemoteLock() {
        	...
        	function loadState(state) {
            	if (state == 'pending') {
            		...
            		$('.button.unlock, .button.lock, .button.error').addClass('hide');
            ...
            function updateLockStatus() {
                ...
                $(".unlock.button").toggleClass("active", !isLocked).toggleClass("inactive", isLocked);
                $(".lock.button").toggleClass("active", isLocked).toggleClass("inactive", !isLocked);
            }
        ...
        initFindMyCar        : function initFindMyCar() {
            ...
            $('.kh-button-submit','.kh-modal-widget-prompt').click(function() {   
            	submitRequestOnClick();
            });
            
            $('.kh-button-retry','.kh-modal-widget-error').click(function() {   
            	submitRequestOnClick();
            });
            ...
            function loadState(state) {
            	if (state == 'pending') {
            		isPending = true;
            		$('.kh-modal-widget').addClass('hide');
                	$('.kh-modal-widget-pending').removeClass('hide');
                	$('.page-header ul li a').addClass('disabled');
                	
                	$('#a_normal span').text('');
                	
                	toggleOnBeforeUnload(true);
            	}
            	else if (state == 'error' || state == 'success') {
            		isPending = false;
            		$('.kh-modal-widget').addClass('hide');
            		$('.page-header ul li a').removeClass('disabled');
            		toggleOnBeforeUnload(false);
            		
            		if (state == 'error') $('.kh-modal-widget-error').removeClass('hide');
            		if (state == 'success') $('.kh-modal-background').addClass('hide');
            	}
            }
            
            function toggleOnBeforeUnload(on) {
            	if (on) window.onbeforeunload = function() {
            		return 'Leaving will cancel the current request.';      		
            	};
                else {
            ...
            function drawMapForCar(carLocation) {            	
                ...
                centerMark = new gMaps.Marker({
                    map      : resultMap,
                    position : carlatLng,
                    icon     : '/images/kh/img/remote/car-pin.png'
                });
                function togglePin() {
            ...
            function sendRequest() {
            	...
            	$.ajax({
            		...
            		error: function(xhr, status, err) {
    	            	...
    	            	$('.kh-modal-widget-error p').text(errorMsg);
    	            	uvo.genericErrorHandler(xhr, status, err);
            		}
            	});            	
            }
