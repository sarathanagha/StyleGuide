(function (uvo) {
    ...
    module.init = function () {
    	...
    	var drivingActivityType = ($('#khModel').val() ? "drivingActivityKh" : "drivingActivity");
        uvo.dataRequest(drivingActivityType).done(function (driveStats) {
            //*/
            $("body").show();
            ...
            var mileage = driveStats.currentMileage,
                ...
                awardCount = driveStats.numAwards;
            ...
            $.each(periods, function (index, period) {
                ...
                if ($('#khModel').val()) { //detect if model is KH to use activateMilestone logic from KH
                	if (index > 1) {
                        activateMilestone(period, (index - 2));
                    }
                } else { //detect if model is non-KH to use activateMilestone logic from CommonPage
                	if (index < 4) {
                        activateMilestone(period, (3 - index));
                    }
                }
            ...
            if($('#khModel').val()) 			//detect if model is KH to use drawSafetyScore logic from KH
            	drawSafetyScore(periods[periods.length-1]);
            else 								//detect if model is non-KH to use drawSafetyScore logic from KH
            	drawSafetyScore(periods[0]); 
            
            var oldest = periods[0];
            $(".idle-time .w-footer strong").text(oldest.moment.format("M/YYYY"));
            
            if(periods.length >= 1)
            	$(".idle-time .amount.day").prepend(totalIdle.toFixed(1));
            else
            	$(".idle-time .amount.day").prepend("0");
...
//                console.log(chartOptions);
                uvo.lazyChart($("#peak-hours"), chartOptions);
